...
public class HttpHelper {
    ...
    private LinkedList<BasicHeader> headers = new LinkedList<BasicHeader>();

    private final int MAX_CONNECTION_COUNT = 150;
    private final int CONNECT_TIMEOUT = 1000;
    private final int SOCKET_TIMEOUT = 3000;
    private final int CONNECTION_REQUEST_TIMEOUT = 1000;

    private final int WAIT_MILLI_SECONDS = 100;
    ...
    private HttpResponse doHttpCall(HttpClient httpclient, HttpUriRequest httpRequest, int maxRetry) throws Exception {
        ...
        while (retryCount <= maxRetry) {
            try {
                ...
                if (needRetry) {
                    //随着重试次数增多，sleep时间变长
                    Thread.sleep(retryCount * WAIT_MILLI_SECONDS);
                    retryCount++;
                } else {
                    throw ex;
                }
